...
export const Login =() => {
    ...
    const handleSub = async (event) => {
        ...
        try {
            const request = await axios.post(`http://localhost:5555/login`, {
                email : email ,
                password : password ,
                create : `${date.getHours()} : ${date.getMinutes()} : ${date.getDate()} : ${date.getDay()} login At `
            })
            ...
            console.log(error)
            alert("account error not Defined")
        }
    }
        ...
        <>
            <div className="mt-3">
                <form onSubmit={handleSub}>
                    <h1 className="text-center my-5">Accountga Login Qilish</h1>
                    ...
                    <button style={{marginLeft:"555px"}} className="btn btn-primary w-25">Login Qilish</button>
                </form>
           </div>
        </>
    )
